...
import com.quizarena.gameservice.game.dto.GameFinishedResponse;
...
public class GameService {
	...
	public Game getGame(final UUID uuid, final Jwt jwt) {
		...
		gameValidationService.validateUserInGame(
			game,
			UserSessionService.getLoggedInUserId(jwt)
		);
		return game;
	...
	public Game addPlayerToGame(String roomCode, Jwt jwt) {
		...
		boolean isAlreadyInGame = game
			.getPlayers()
			.stream()
			.anyMatch(player -> player.getUserId().equals(userId));
		if (isAlreadyInGame) {
			throw new IllegalStateException(
				"User is already a participant of this game"
			);
		}
	...
	private void processQuestionStep(UUID gameId, String roomCode) {
		...
		gameNotificationService.notifyGame(
			...
			QuestionResponse.from(
				...
				game.getRound(),
				game.getQuiz().getQuestionsCount(),
				game.getAnswerTimeInSeconds(),
				game.getStartGameTime()
			)
		);
		taskScheduler.schedule(
			() -> handleEndOfQuestion(gameId, roomCode),
	...
	private void finishGame(UUID gameId, String roomCode) {
		...
		gameRepository.saveGame(game);
		userServiceClient.sendGameResults(
			GameResultResponse.from(game.getPlayers())
		);
		gameNotificationService.notifyGame(
			roomCode,
			GameEventType.GAME_FINISHED,
			GameFinishedResponse.builder().gameId(gameId).build()
		);
	}
}
